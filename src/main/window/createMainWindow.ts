...
export function createMainWindow(
  ...
  mainWindow.webContents.on('zoom-changed', (event, zoomDirection) => {
    // Why: some layouts fire Electron's zoom command without before-input-event; honor it only while the zoom action is still bound. 'reset' is included because Electron can handle Cmd+0 natively first.
